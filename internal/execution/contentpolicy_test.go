...
// Both boundaries have to be given the document and the person, not just the
// scanner's settings. A guard built from the DLP row alone leaves every selector
// about a person empty, and an empty selector matches nothing — the rule then
// falls out silently and the class's global action decides, which is the exact
// shape of the bug this pair of tests was written for.
func TestBothSendersAreGivenThePolicyAndTheOwner(t *testing.T) {
	for _, boundary := range []struct{ file, builder string }{
		{"provenance.go", "func (d *Dispatcher) contentGuard("},
		{"scmpost.go", "func (o *Orchestrator) contentGuard("},
	} {
		...
		source := string(body)
		at := strings.Index(source, boundary.builder)
		if at < 0 {
			t.Fatalf("%s no longer builds a guard the way this test reads it", boundary.file)
		}
		builder := source[at:]
		if end := strings.Index(builder, "\n}\n"); end > 0 {
			builder = builder[:end]
		}
		for _, want := range []struct{ needle, missing string }{
			{"dlp.SettingKey", "does not read the scanner's settings from where the console writes them"},
			{"policy.SettingKey", "does not read the central policy, so a rule can only ever be a global action"},
			{"UserByID(", "never looks up who this belongs to, so a rule about a role or a person matches nothing"},
		} {
			if !strings.Contains(builder, want.needle) {
				t.Errorf("%s %s", boundary.file, want.missing)
			}
		}
	}
}
