...
func (s *Server) apiRoutes() []Route {
	return []Route{
		...
		browser(http.MethodGet, "/scm-connections", "Developer", "List the caller's forge connections", s.scmConnections),
		browser(http.MethodPost, "/scm-connections", "Developer", "Store a forge credential", s.putSCMConnection),
		browser(http.MethodDelete, "/scm-connections/{id}", "Developer", "Delete a forge connection", s.deleteSCMConnection),
		...
		admin(http.MethodGet, "/admin/mail/deliveries", "Administration", "What left the building by mail: every attempt, sent or not", s.adminMailDeliveries),
		admin(http.MethodPost, "/admin/mail/test", "Administration", "Send one test mail through the saved relay settings", s.adminSendTestMail),
		...
		admin(http.MethodPost, "/admin/provenance/test", "Administration", "Send one sample decision record to the configured address", s.testProvenance),
