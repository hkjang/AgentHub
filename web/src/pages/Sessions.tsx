...
export function Sessions() {
  ...
  ]).catch((e) => { setSessions([]); setError(e instanceof Error?e.message:'Session 목록을 불러오지 못했습니다.') })
  useEffect(() => { void load() }, [])
  ...
  return <div className="page">
    <PageHeader eyebrow="AGENT WORKSPACE" title="Runtime Sessions" description="Agent별 작업 맥락과 실행 진입점을 분리해 관리합니다." actions={<button className="button primary" disabled={available.length===0} title={available.length===0?'Running 또는 Ready 상태의 Runtime이 있어야 Session을 시작할 수 있습니다.':undefined} onClick={() => setOpen(true)}><Plus size={16}/>새 Session</button>}/>
    {error&&<ErrorBanner message={error} onClose={()=>setError('')}/>}
    {sessions.length===0?<Empty icon={<FileCode2/>} title="Session이 없습니다" description={available.length?'실행 중인 Runtime을 선택해 첫 작업 Session을 시작하세요.':'먼저 My Agents에서 Runtime을 시작한 뒤 Session을 만들 수 있습니다.'} action={available.length?<button className="button primary" onClick={()=>setOpen(true)}>Session 시작</button>:undefined}/>:<section className="session-list">{sessions.map((item) => <article key={item.id}><div className={runtimeLogoClass(item.runtimeType)}>{runtimeCode(item.runtimeType)}</div><div><div><h3>{item.title}</h3><StatusBadge status={item.status}/></div><p>{item.agentName} · {runtimeLabel(item.runtimeType)}</p><span>{new Date(item.updatedAt).toLocaleString('ko-KR')} · Trace {Array.isArray(item.trace)?item.trace.length:0}</span></div><div className="session-actions">{item.status==='active'&&<button className="button primary" onClick={()=>void launch(item.runtimeId)}><ExternalLink size={15}/>열기</button>}{item.status==='active'&&<button className="button ghost" onClick={()=>void close(item.id)}><Square size={14}/>종료</button>}</div></article>)}</section>}
