        ...
        <label><span>결과</span>
          <select value={auditQuery.outcome} onChange={(e) => search({ outcome: e.target.value })}>
            ...
            {/* What a content scan did: 차단됨 held the call back, 가림 처리됨 rewrote
                the text, 기록만 let it through untouched, 일부 미검사 means the payload
                was longer than the scan limit and only its beginning was read. */}
            <option value="blocked">차단됨</option><option value="redacted">가림 처리됨</option><option value="audited">기록만</option><option value="unscanned">일부 미검사</option>
          </select>
        </label>
