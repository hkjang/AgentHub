...
export function Catalog({builder=false}:{builder?:boolean}){
  ...
  return <div className="page"><PageHeader eyebrow={builder?'AGENT FACTORY':'MARKETPLACE'} title={builder?'Agent Builder':'Agent Catalog'} description={builder?'Template, Runtime Profile과 Workspace를 조합해 개인 Agent를 만듭니다.':'관리자가 검증하고 게시한 Agent Template으로 빠르게 시작하세요.'}/>
    <div className="toolbar"><div className="search-box"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Template 또는 Runtime 검색"/></div><div className="filter-chips"><button className={activeCategory==='전체'?'selected':''} onClick={()=>setActiveCategory('전체')}>전체</button><button className={activeCategory==='Development'?'selected':''} onClick={()=>setActiveCategory('Development')}>Development</button><button className={activeCategory==='Automation'?'selected':''} onClick={()=>setActiveCategory('Automation')}>Automation</button><button className={activeCategory==='Research'?'selected':''} onClick={()=>setActiveCategory('Research')}>Research</button><button className={activeCategory==='Operations'?'selected':''} onClick={()=>setActiveCategory('Operations')}>Operations</button></div></div>
    {templates.length===0?<Loading/>:filtered.length===0?<div className="empty-compact">검색 조건에 맞는 Template이 없습니다.</div>:<div className="catalog-grid">{filtered.map(t=><button className="template-card" key={t.id} onClick={()=>setSelected(t)}><div className="template-top"><div className={runtimeLogoClass(t.runtimeType,'large')}>{runtimeCode(t.runtimeType)}</div><span className="verified"><Check size={12}/>Verified</span></div><span className="category">{t.category}</span><h3>{t.name}</h3><p>{t.description}</p><div className="template-meta"><span><Bot size={14}/>{runtimeLabel(t.runtimeType)}</span><span><Boxes size={14}/>v{t.version}</span></div></button>)}</div>}
    ...
    try{
      ...
      if(autoStart&&agentId){
        try{
          ...
          setError(`Agent는 저장되었지만 Runtime을 시작하지 못했습니다: ${spawnErr instanceof Error?spawnErr.message:'알 수 없는 오류'}`)
          setBusy(false)
          return
        }
      }
