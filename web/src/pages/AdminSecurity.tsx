...
export function AdminSecurity() {
  ...
  return <div className="page">
    <PageHeader eyebrow="ADMINISTRATION" title="Security & Network Profiles" description="Kubernetes 세부 옵션을 숨기고 승인된 격리·Egress 정책만 Runtime Template에 제공합니다." actions={<button className="button primary" onClick={() => setSelected(null)}><Plus size={16}/>새 Profile</button>}/>
...
function PolicyDrawer({kind,item,close,done,error}:{kind:Kind;item:Profile|null;close:()=>void;done:()=>void;error:(value:string)=>void}) {
  ...
  return <Drawer title={`${item?'수정':'새로 등록'} · ${kind === 'security'?'Security':'Network'} Profile`} close={close} footer={<><button className="button ghost" onClick={close}>취소</button><button className="button primary" form="policy-form" disabled={busy}>{busy?'저장 중…':'저장'}</button></>}><form id="policy-form" className="drawer-form" onSubmit={submit}><label><span>이름</span><input required maxLength={80} value={name} onChange={(e) => setName(e.target.value)}/></label><label><span>설명</span><textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)}/></label>{kind === 'security'?<><LockedToggle label="Non-root 실행" value/><Toggle label="Read-only Root Filesystem" value={Boolean(spec.readOnlyRootFilesystem)} change={(v) => update('readOnlyRootFilesystem',v)}/><LockedToggle label="Privilege Escalation 차단" value/><LockedToggle label="ServiceAccount Token 차단" value/><label><span>Seccomp Profile</span><select value="RuntimeDefault" disabled><option>RuntimeDefault</option></select></label><div className="info-box"><LockKeyhole size={17}/><div><strong>보안 불변조건</strong><p>Non-root, 권한 상승·Kubernetes Token 차단, RuntimeDefault seccomp와 Capability 전체 제거는 완화할 수 없습니다.</p></div></div></>:<><Toggle label="Default deny Egress" value={Boolean(spec.defaultDeny)} change={(v) => update('defaultDeny',v)}/><Toggle label="Cluster DNS 허용" value={Boolean(spec.allowDNS)} change={(v) => update('allowDNS',v)}/><label><span>허용 Destination</span><textarea rows={6} value={(spec.allowedDestinations as string[]??[]).join('\n')} onChange={(e) => update('allowedDestinations',e.target.value.split('\n').map((v) => v.trim()).filter(Boolean))} placeholder={'10.20.0.0/16:443\n[fd00:20::/64]:8080'}/><small>Kubernetes NetworkPolicy가 지원하는 CIDR:port 형식으로 한 줄에 하나씩 입력합니다.</small></label></>}<Toggle label="Profile 사용" value={enabled} change={setEnabled}/></form></Drawer>
}
