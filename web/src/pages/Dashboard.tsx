...
export function Dashboard(){
  ...
  return <div className="page"><PageHeader eyebrow="CONTROL PLANE" title={`${user.displayName}님, 안녕하세요`} description="Agent Runtime과 Workspace 상태를 한눈에 확인하세요." actions={<Link className="button primary" to="/catalog"><Plus size={17}/>새 Agent</Link>}/>
    ...
    <div className="dashboard-grid"><section className="panel"><div className="panel-header"><div><h2>최근 Agent</h2><p>현재 Runtime 상태와 마지막 활동</p></div><Link to="/agents">모두 보기<ArrowUpRight size={15}/></Link></div><div className="agent-list">{agents.length===0?<div className="empty-compact">아직 생성한 Agent가 없습니다. Catalog에서 시작해 보세요.</div>:agents.map((agent)=><Link to="/agents" className="agent-row" key={agent.id}><div className={`runtime-logo ${agent.runtimeType}`}>{getRuntimeCode(agent.runtimeType)}</div><div className="agent-main"><strong>{agent.name}</strong><span>{agent.runtimeType} · v{agent.version}</span></div><StatusBadge status={agent.runtime?.status??'stopped'}/><span className="row-time"><Clock3 size={14}/>{new Date(agent.updatedAt).toLocaleDateString('ko-KR')}</span></Link>)}</div></section>
