...
import { api } from '../api'
import { safeReturnTo, type AuthMethods } from '../silentSso'
import type { Version } from '../types'
import { ErrorBanner } from '../components/UI'

// The callback lands here with ?sso=error when the provider answered with an
// error that was not the silent attempt's ordinary "no session" (that one is
// ?sso=none and says nothing — it is the reason this screen is showing).
function providerNote(search: string): string {
  return new URLSearchParams(search).get('sso') === 'error' ? 'SSO 로그인이 완료되지 않았습니다. 다시 시도하거나 관리자에게 문의하세요.' : ''
}

export function Login({version,onLogin}:{version:Version;onLogin:()=>Promise<unknown>}) {
  const [methods,setMethods]=useState<AuthMethods>({local:true,oidc:false,oidcLabel:'Keycloak SSO'})
  const [username,setUsername]=useState(''),[password,setPassword]=useState(''),[error,setError]=useState(providerNote(window.location.search)),[busy,setBusy]=useState(false)
  useEffect(()=>{api.get<AuthMethods>('/api/v1/auth/methods').then(setMethods).catch(()=>undefined)},[])
  // A person who opened a deep link and signs in by the button goes back to it.
  const ssoHref=`/api/v1/auth/oidc/start?return_to=${encodeURIComponent(safeReturnTo(window.location.pathname+window.location.search+window.location.hash))}`
  ...
  return <main className="login-page">
    ...
    <section className="login-form-side"><div className="login-card"><div className="login-heading"><div className="login-lock"><LockKeyhole size={22}/></div><h2>AgentHub에 로그인</h2><p>조직 계정으로 Runtime Workspace에 접속합니다.</p></div>{error&&<ErrorBanner message={error}/>}
      {methods.oidc&&<><a className="button primary wide" href={ssoHref}>{methods.oidcLabel}<ArrowRight size={17}/></a>{methods.local&&<div className="separator"><span>또는 로컬 관리자 계정</span></div>}</>}
