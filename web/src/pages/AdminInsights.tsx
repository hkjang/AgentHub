...
function attention(overview: Overview) {
  ...
  // Every number under this one is a single figure in a single currency, and the
  // price table is not: two endpoints in different currencies were added together
  // and labelled with whichever string sorted highest.
  if ((overview.spend.currencies?.length ?? 0) > 1) items.push({
    level: 'danger', to: '/admin/models',
    text: `이 금액은 서로 다른 통화(${overview.spend.currencies!.join(', ')})를 그대로 더한 값입니다. 비용 예산도 같은 계산을 쓰므로 엔드포인트 통화를 하나로 맞춰 주세요.`
  })
...
export function AdminInsights() {
  ...
  return <div className="page">
    ...
    <section className="kpi-row">
      ...
      <article><span>비용</span><strong>{(spend.currencies?.length ?? 0) > 1 ? `${number(spend.cost)} (통화 혼재)` : money(spend.cost, spend.currency)}</strong><small>{spend.unpricedTokens > 0 ? `미산정 ${number(spend.unpricedTokens)} 토큰` : spend.unmeteredRuns > 0 ? `집계 안 된 실행 ${number(spend.unmeteredRuns)}건` : '전 구간 단가 적용'}</small></article>
    </section>
