...
import { Boxes, ExternalLink, FileCog, KeyRound, Network, Plus, Save, Settings, ShieldCheck, Trash2 } from 'lucide-react'
import { api } from '../api'
...
function RuntimeEnvironmentForm({value,update}:{value:Record<string,unknown>;update:(key:string,v:unknown)=>void}){
  ...
  return <>
    <Section title="공통 파일" description="활성화된 파일은 모든 Runtime Pod의 모든 컨테이너에 같은 경로로 읽기 전용 마운트됩니다.">
      <div className="info-box"><FileCog size={17}/><div><strong>ConfigMap으로 전달됩니다</strong><p>비밀값은 넣지 마세요. 저장 후 다음 Runtime 시작 또는 재시작 시점에 적용됩니다. /etc/agenthub, /usr/local/bin 등 플랫폼이 쓰는 경로는 사용할 수 없습니다.</p></div></div>
