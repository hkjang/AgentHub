...
export function MCPFabric({view}:{view:'catalog'|'bundles'}) {
  ...
  return <div className="page">
    <PageHeader eyebrow="MCP 패브릭" title={catalog?'MCP 카탈로그':'MCP 번들'} description={catalog?'Agent Runtime에 연결할 수 있는 활성 MCP와 실행 격리 방식을 확인합니다.':'업무 목적별 MCP 조합을 Agent Builder에서 한 번에 선택합니다.'}/>
    {error&&<ErrorBanner message={error}/>}
    {catalog ? (servers.length === 0 ? <Empty icon={<Network/>} title="사용 가능한 MCP가 없습니다" description="관리자가 MCP Server를 활성화하면 이곳에 표시됩니다."/> : <section className="mcp-grid">{servers.map((item) => <article className="mcp-card" key={item.id}><header><div className="list-icon"><Network/></div><StatusBadge status={item.mode}/></header><h3>{item.name}</h3><p>{item.description || 'MCP 도구 연결'}</p><dl><div><dt>전송 방식</dt><dd>{item.transport}</dd></div><div><dt>위험도</dt><dd>{item.riskLevel}</dd></div><div><dt>엔드포인트</dt><dd>{item.mode === 'shared' ? safeHost(item.endpoint) : `Pod :${item.port}`}</dd></div></dl>{item.approvalRequired&&<footer><ShieldAlert size={15}/>도구 실행 전 승인 필요</footer>}</article>)}</section>) :
