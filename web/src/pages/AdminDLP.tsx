...
export function AdminDLP() {
  ...
  return <div className="page">
    <PageHeader eyebrow="관리자 · 거버넌스" title="내용 검사 (DLP)"
      description="모델 호출과 MCP 도구 호출, 밖으로 나가는 결정 기록과 리뷰 코멘트에 포함된 개인정보·신용정보·자격증명을 찾아 가리거나 차단합니다. 찾은 값은 어디에도 저장하지 않습니다."
    ...
    <GuidePanel id="admin-dlp" title="어떻게 동작하나요" steps={[
      { title: '검사 지점', body: <>모델로 나가는 프롬프트는 컨트롤 플레인이, MCP 도구 호출은 Pod 안 게이트웨이가 검사하며, <Link to="/admin/provenance">결정 기록 내보내기</Link>와 코드 리뷰 결과를 원래 Pull Request에 남기는 코멘트도 나가기 전에 같은 설정으로 검사합니다. 도구 호출은 컨트롤 플레인을 거치지 않기 때문에, 에이전트가 우회할 수 없는 자리에서 검사해야 의미가 있습니다.</> },
