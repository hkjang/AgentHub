...
export type UsageReport = { from:string; to:string; currency:string; inputTokens:number; outputTokens:number; cost:number; unpricedTokens:number; runs:number; unmeteredRuns:number; unrecordedRuns?:number; unrecordedTokens?:number; agents:UsageRow[]; daily:UsagePoint[] }
