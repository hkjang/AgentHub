...
export type Workspace = {
  ...
  status: string;
  updatedAt: string;
  gitCredentialSecretId?: string;
  gitCredentialKind?: "" | "token" | "ssh-key";
...
export type Workflow = {
  ...
  definition: { steps: { id: string; agentId: string; dependsOn: string[] }[] };
  enabled: boolean;
  updatedAt: string;
};
export type EvaluationTestSet = {
  id: string;
  name: string;
  description: string;
...
export type AgentEvaluation = {
  ...
  result: { cases: { name: string; passed: boolean; failures: string[] }[] };
  createdAt: string;
};
...
export type SupervisionRound = {
  round: number;
  approved: boolean;
  revisions: RevisionRequest[] | null;
};
export type SupervisionResult = {
  supervisor: string;
  approved: boolean;
  rounds: SupervisionRound[];
  exhausted: boolean;
};
...
export type WorkflowRun = {
  ...
  workflowId: string;
  status: string;
  input: { input?: string; mode?: string };
...
export type ExecutionMode =
  "interactive" | "task" | "scheduled" | "event" | "service" | "hybrid";
...
export type Department = {
  ...
  name: string;
  description: string;
  quota: { perMember: Limits; total: Limits };
  members: number;
  held: Held;
  createdAt: string;
  updatedAt: string;
...
export type EffectiveQuota = {
  ...
  effective: Limits;
  held: Held;
  departmentQuota: { perMember: Limits; total: Limits };
  departmentHeld: Held;
};
...
export type AgentServer = {
  ...
  enabled: boolean;
  running?: number;
  health: "unknown" | "healthy" | "unreachable" | "refused";
  healthDetail?: string;
  checkedAt?: string;
  createdAt: string;
  updatedAt: string;
 ...
 *  addresses. */
export type UsableAgentServer = {
  id: string;
  name: string;
  networkZone: string;
  health: AgentServer["health"];
  kind: string;
};
...
export type ReviewPlan = {
  mode: string;
  repository: string;
  reviewable: ReviewPlanFile[];
  excluded: ReviewPlanFile[];
  insertions: number;
  deletions: number;
  groups: ReviewPlanGroup[];
};

// A credential for a forge, without the credential. lastError is the difference
// between a review that found nothing and a token somebody revoked.
