...
- 🏷️ **Agent 버전 관리와 운영 승격**: 저장할 때마다 그 시점의 정의가 버전으로 남고, 사전검증 결과는 검사한 버전에 기록됩니다. 통과한 버전만 운영 승격할 수 있으며(관리자는 사유를 남겨야 건너뜁니다), *승격된 정의만 실행* 을 켜면 승격되지 않은 편집으로는 작업이 생성되지도 실행되지도 않습니다. 잘못된 편집은 이전 정의를 **새 버전으로 복원** 해 되돌립니다.
...
minikube image load agenthub:v0.16.0
