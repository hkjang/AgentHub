...
- 감사에는 **값이 아니라 등급·건수·조치와 마스킹된 샘플만** 남습니다. `dlp.tool` 은 Pod 가
  보고한 것입니다 — 발견의 등급·라벨·샘플·조치는 컨트롤 플레인이 자기 것으로 바꿔
  저장하지만(샘플은 그 등급의 규칙으로 **다시 마스킹**, 모르는 등급은 `unknown` 으로,
  라벨은 등급에서 찾아서), 어느 서버·툴·방향의 호출이었는지는 Pod 가 말한 그대로
  실립니다. 즉 Pod 가 샘플 자리에 무엇을 보내든 트레일에 값이 실리지 않지만, 서버·툴
  이름 자리는 Pod 의 말입니다.
