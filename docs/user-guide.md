...
4. **게이트를 켜면 승격본만 실행됩니다** — *승격된 정의만 실행* 을 켜 두면 현재 정의가 승격본과 다를 때 작업 생성이 409로 즉시 거절되고, 이미 대기 중이던 작업도 워커가 같은 사유로 실패 처리합니다. 게이트는 **기본값이 꺼짐** 이므로 켜지 않은 에이전트는 지금까지와 똑같이 동작합니다.
